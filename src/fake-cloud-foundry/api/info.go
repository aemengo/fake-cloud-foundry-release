...
func (a *API) Info(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	info := map[string]interface{}{
		...
		"version":                      0,
		"description":                  "",
		"authorization_endpoint":       fmt.Sprintf("http://uaa.%s", a.config.Domain()),
		"token_endpoint":               fmt.Sprintf("http://uaa.%s/oauth/token", a.config.Domain()),
		"min_cli_version":              nil,
		"min_recommended_cli_version":  nil,
		"app_ssh_endpoint":             fmt.Sprintf("ssh.%s", a.config.Domain()),
		"app_ssh_host_key_fingerprint": "",
		"app_ssh_oauth_client":         "ssh-proxy",
		"doppler_logging_endpoint":     fmt.Sprintf("wss://doppler.%s", a.config.Domain()),
		"api_version":                  "999.999.999",
		"osbapi_version":               "2.14",
		"routing_endpoint":             fmt.Sprintf("http://api.%s/routing", a.config.Domain()),
	}
